...
## `report.add_df(name, df, format="records", compress=False, timestamp_format="iso")`
...
sort/filter/format type-aware: booleans → `boolean`, datetimes → `date`,
numerics → `number`, everything else → `string`.
